...
marianas_access : $(foreach sample,$(SAMPLES),marianas/$(sample)/$(sample)_R1.fastq.gz) \
...
define fastq-to-bam
marianas/$1/$1_cl_aln.bam : marianas/$1/$1_R1_umi-clipped.fastq.gz
...
marianas/$1/$1_cl_aln_srt.bam : marianas/$1/$1_cl_aln.bam
	$$(call RUN,-c -n $(SAMTOOLS_THREADS) -s 1G -m $(SAMTOOLS_MEM_THREAD),"set -o pipefail && \
									       ...
									       $$(SAMTOOLS) index $$(@) && \
									       cp marianas/$1/$1_cl_aln_srt.bam.bai marianas/$1/$1_cl_aln_srt.bai")
									  									   		   
marianas/$1/$1_cl_aln_srt_MD.bam : marianas/$1/$1_cl_aln_srt.bam
...
marianas/$1/$1_cl_aln_srt_MD_IR_FX.grp : marianas/$1/$1_cl_aln_srt_MD_IR_FX.bam
...
marianas/$1/$1_cl_aln_srt_MD_IR_FX_BR_RG.bam : marianas/$1/$1_cl_aln_srt_MD_IR_FX_BR.bam
	$$(call RUN, -c -n 1 -s 12G -m 18G,"set -o pipefail && \
					    $$(ADD_RG) \
					    INPUT=$$(<) \
					    OUTPUT=$$(@) \
					    ...
					    RGSM=$1 && \
					    $$(SAMTOOLS) index $$(@) && \
					    cp marianas/$1/$1_cl_aln_srt_MD_IR_FX_BR_RG.bam.bai marianas/$1/$1_cl_aln_srt_MD_IR_FX_BR_RG.bai")
...
metrics/summary/umi_frequencies.tsv : $(foreach sample,$(SAMPLES),marianas/$(sample)/family-sizes.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/umi_metrics.R --type 1 --sample_names '$(SAMPLES)'")
	
metrics/summary/umi_composite.tsv : $(foreach sample,$(SAMPLES),marianas/$(sample)/family-sizes.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/umi_metrics.R --type 2 --sample_names '$(SAMPLES)'")

metrics/summary/umi_families.tsv : $(foreach sample,$(SAMPLES),marianas/$(sample)/family-sizes.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/umi_metrics.R --type 3 --sample_names '$(SAMPLES)'")

metrics/standard/metrics_idx.tsv : $(foreach sample,$(SAMPLES),metrics/standard/$(sample).idx_stats.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 1 --sample_names '$(SAMPLES)'")
									  
metrics/unfiltered/metrics_idx.tsv : $(foreach sample,$(SAMPLES),metrics/unfiltered/$(sample).idx_stats.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 7 --sample_names '$(SAMPLES)'")

metrics/duplex/metrics_idx.tsv : $(foreach sample,$(SAMPLES),metrics/duplex/$(sample).idx_stats.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 12 --sample_names '$(SAMPLES)'")

metrics/simplex/metrics_idx.tsv : $(foreach sample,$(SAMPLES),metrics/simplex/$(sample).idx_stats.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
...
metrics/standard/metrics_insert.tsv : $(foreach sample,$(SAMPLES),metrics/standard/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 3 --sample_names '$(SAMPLES)'")

metrics/unfiltered/metrics_insert.tsv : $(foreach sample,$(SAMPLES),metrics/unfiltered/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 9 --sample_names '$(SAMPLES)'")

metrics/duplex/metrics_insert.tsv : $(foreach sample,$(SAMPLES),metrics/duplex/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 14 --sample_names '$(SAMPLES)'")

metrics/simplex/metrics_insert.tsv : $(foreach sample,$(SAMPLES),metrics/simplex/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 19 --sample_names '$(SAMPLES)'")

metrics/standard/metrics_insert_distribution.tsv : $(foreach sample,$(SAMPLES),metrics/standard/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 16G -m 24G,"set -o pipefail && \
					   $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 4 --sample_names '$(SAMPLES)'")
									   
metrics/unfiltered/metrics_insert_distribution.tsv : $(foreach sample,$(SAMPLES),metrics/unfiltered/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 16G -m 24G,"set -o pipefail && \
					   $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 10 --sample_names '$(SAMPLES)'")

metrics/duplex/metrics_insert_distribution.tsv : $(foreach sample,$(SAMPLES),metrics/duplex/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 16G -m 24G,"set -o pipefail && \
					   $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 15 --sample_names '$(SAMPLES)'")

metrics/simplex/metrics_insert_distribution.tsv : $(foreach sample,$(SAMPLES),metrics/simplex/$(sample).insert_metrics.txt)
	$(call RUN, -c -n 1 -s 16G -m 24G,"set -o pipefail && \
					   $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 20 --sample_names '$(SAMPLES)'")
									   
metrics/standard/metrics_hs.tsv : $(foreach sample,$(SAMPLES),metrics/standard/$(sample).probe-A.hs_metrics.txt) $(foreach sample,$(SAMPLES),metrics/standard/$(sample).probe-B.hs_metrics.txt) $(foreach sample,$(SAMPLES),metrics/standard/$(sample).probe-A.hs_metrics-nodedup.txt) $(foreach sample,$(SAMPLES),metrics/standard/$(sample).probe-B.hs_metrics-nodedup.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 6 --sample_names '$(SAMPLES)'")
	
metrics/unfiltered/metrics_hs.tsv : $(foreach sample,$(SAMPLES),metrics/unfiltered/$(sample).probe-A.hs_metrics.txt) $(foreach sample,$(SAMPLES),metrics/unfiltered/$(sample).probe-B.hs_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 11 --sample_names '$(SAMPLES)'")
	
metrics/duplex/metrics_hs.tsv : $(foreach sample,$(SAMPLES),metrics/duplex/$(sample).probe-A.hs_metrics.txt) $(foreach sample,$(SAMPLES),metrics/duplex/$(sample).probe-B.hs_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 16 --sample_names '$(SAMPLES)'")
	
metrics/simplex/metrics_hs.tsv : $(foreach sample,$(SAMPLES),metrics/simplex/$(sample).probe-A.hs_metrics.txt) $(foreach sample,$(SAMPLES),metrics/simplex/$(sample).probe-B.hs_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
					  $(RSCRIPT) $(SCRIPTS_DIR)/qc/interval_metrics.R --metric_type 21 --sample_names '$(SAMPLES)'")
									  
metrics/standard/metrics_oxog.tsv : $(foreach sample,$(SAMPLES),metrics/standard/$(sample).oxog_metrics.txt)
	$(call RUN, -c -n 1 -s 8G -m 12G,"set -o pipefail && \
...
metrics/summary/metrics_ts.tsv : $(foreach sample,$(SAMPLES),metrics/standard/$(sample).A.ontarget.txt) $(foreach sample,$(SAMPLES),metrics/standard/$(sample).B.ontarget.txt) $(foreach sample,$(SAMPLES),metrics/standard/$(sample).AB.offtarget.txt)
	$(call RUN, -c -n 1 -s 8G -m 16G,"set -o pipefail && \
...
metrics/report/non_reference_calls.pdf : $(foreach sample,$(SAMPLES),metrics/standard/$(sample)-pileup.txt) $(foreach sample,$(SAMPLES),metrics/simplex/$(sample)-pileup.txt) $(foreach sample,$(SAMPLES),metrics/duplex/$(sample)-pileup.txt)
...
..DUMMY := $(shell mkdir -p version; \
	     $(BWA) &> version/tmp.txt; \
	     head -3 version/tmp.txt | tail -2 > version/marianas_access.txt; \
	     rm version/tmp.txt; \
	     $(SAMTOOLS) --version >> version/marianas_access.txt; \
	     echo "gatk3" >> version/marianas_access.txt; \
	     $(GATK) --version >> version/marianas_access.txt; \
	     echo "picard" >> version/marianas_access.txt; \
	     $(PICARD) MarkDuplicates --version &>> version/marianas_access.txt; \
	     R --version >> version/marianas_access.txt)
.DELETE_ON_ERROR:
.SECONDARY:
.PHONY: marianas_access
