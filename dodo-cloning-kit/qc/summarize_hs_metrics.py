...
def slack_warning(slack_group, slack_token, slack_channel, message):
	slack_url = os.getenv('SLACK_URL_QC')
	slack_user = "@"+os.getenv('USER')
	slack_message = "curl -X POST -H 'Content-type: application/json' --data '{\"text\":\""+slack_user+" "+message+"\"}' "+slack_url+" &> /dev/null"
	post_code = os.system(slack_message)
...
if __name__ == "__main__":
    ...
    if len(below_threshold) > 0:
        ...
        warning_message = ":point_up: <{}X cov: {} : {}".format(args.depth_threshold, samples, os.getcwd())
