...
for (i in 1:length(sample_names)) {
	fusion_summary[[i]] = read_tsv(file=paste0("defuse/", sample_names[i], ".results.filtered.tsv"), col_types = cols(.default = col_character())) %>%
		   			 	  type_convert() %>%
		   			 	  mutate(sample_id = sample_names[i])
}
fusion_summary = do.call(rbind, fusion_summary)
write_tsv(fusion_summary, path="defuse/summary.tsv")
