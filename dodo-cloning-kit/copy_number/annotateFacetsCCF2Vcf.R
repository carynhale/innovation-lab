...
if (sum(pass) == 0) {
    ...
    ol[is.na(ol)] = nearest(rowRanges(vcf[pass])[is.na(ol)], facetsGr)
    if (sum(!is.na(ol)) > 0) {
        ...
        conf = conf_ccf(alt = alt, ref = ref, tcn, lcn, purity = purity, multiplicity = ccfFit$multiplicity)
        ccfLower = conf$lower
        ccfUpper = conf$upper
        clonalStatus = ifelse(round(ccfLower, 2) >= 0.75, "clonal", ifelse(round(ccfLower, 2) < 0.75 & ccfFit$ccf >= 0.8, 'likely_clonal', "subclonal"))
