...
cmp_vcf/grp/%.gt_concord.grp : $(foreach type,$(VCF_TYPES),cmp_vcf/vcf/%.$(VCF_SUFFIX.$(type)).vcf)
	$(call RUN,-s 9G -m 12G,"$(call GATK_MEM,8G) -T GenotypeConcordance --dbsnp $(DBSNP) -R $(REF_FASTA)  --eval:$(<F:.$(FILTER_SUFFIX).vcf=) $< $(foreach i,$(wordlist 2,$(words $^),$^),--comp:$(notdir $(i:.$(FILTER_SUFFIX).vcf=)) $i ) -o $@")
